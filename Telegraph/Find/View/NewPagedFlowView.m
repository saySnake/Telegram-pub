...
- (void)initialize{
     ...
     */
    UIView *superViewOfScrollView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height-20) ];
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (long i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                HBFindAvatarScrollView *cell = [_cells objectAtIndex:i];
...
- (UIView *)dequeueReusableCell{
    HBFindAvatarScrollView *cell = [_reusableCells lastObject];
    if (cell)
    {
