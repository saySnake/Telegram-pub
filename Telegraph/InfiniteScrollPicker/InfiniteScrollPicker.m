...
- (void)initInfiniteScrollView:(ETHModel *)model
{
    ...
    if (_imageAry.count > 0)
    {
        ...
        for (int i = 0; i < (_imageAry.count*5); i++)
        {
            ...
            UIImageView *photo =[[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetMinX(temp.frame)+2, 5, 10, 10)];
            photo.cornerRadius =5;
            photo.image =[UIImage imageNamed:model.imgUrl];
            [temp addSubview:photo];
            
            UILabel *label =[[UILabel alloc] init];
            label.frame =CGRectMake(CGRectGetMaxX(photo.frame), 5, 40, 30);
            label.text =model.name;
            [temp addSubview:label];
            
            UILabel *monLab =[[UILabel alloc] init];
            monLab.frame =CGRectMake(0, CGRectGetMaxY(photo.frame), _itemSize.width, 20);
            monLab.font =kFontSmall;
            [temp addSubview:monLab];
...
- (void)setImageAry:(NSArray *)imageAry model:(ETHModel *)model
{
    _imageAry = imageAry;
    [self initInfiniteScrollView:model];
}

- (void)setItemSize:(CGSize)itemSize
{
    itemSize = itemSize;
    [self initInfiniteScrollView:nil];
}
